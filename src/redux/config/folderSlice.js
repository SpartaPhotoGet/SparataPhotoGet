...
export const __updateTag = createAsyncThunk(
  "UPDATE_TAG",
  async (payload, thunkAPI) => {
    try {
      const result = await axios.patch(
        `http://localhost:3001/contents/tags/${payload.id}`,
        payload
      );
      return thunkAPI.fulfillWithValue(result.data);
    } catch (e) {
...
export const folderSlice = createSlice({
  ...
  extraReducers: {
    ...
    // UPDATE
    [__updateTag.pending]: (state) => {
      state.isLoading = true;
    },
    [__updateTag.fulfilled]: (state, action) => {
      const target = state.tags.findIndex(
        (tag) => tag.id === action.payload.id
      );
      state.tags.splice(target, 1, action.payload);
    },
    [__updateTag.rejected]: (state, action) => {
      state.isLoading = false;
